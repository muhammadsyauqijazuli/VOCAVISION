...
function CardField({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-gray-1 p-4 dark:bg-dark-2">
      <p className="text-xs font-semibold uppercase tracking-[0.18em] text-dark-4 dark:text-dark-6">{label}</p>
      <p className="mt-1 text-sm font-semibold text-dark dark:text-white">{value}</p>
    </div>
  );
}
...
export default async function GuruStudentReportPage({ params }: PageProps) {
  ...
  const [studentResult, insightResult, interventionResult] = await Promise.all([
    ...
    fetchJson<InterventionRecord[]>(`${origin}/api/interventions/${id}`, cookie),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-270 space-y-6">
      ...
      <section className="rounded-2xl border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
        ...
        <h1 className="text-heading-4 mb-2 font-bold text-dark dark:text-white">Report Akademik Per Siswa</h1>
        <p className="max-w-3xl text-sm text-dark-4 dark:text-dark-6">
          Laporan ini merangkum profil satu siswa, prediksi terbaru, faktor yang paling memengaruhi hasil, dan tindak lanjut guru yang sudah dicatat.
        </p>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.65fr]">
        <div className="rounded-2xl border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-primary">Profil siswa</p>
              <h2 className="text-2xl font-bold text-dark dark:text-white">{student.nama}</h2>
          ...
          <p className="mt-4 text-sm leading-6 text-dark-4 dark:text-dark-6">
            File export menyajikan profil siswa, prediksi terbaru, faktor SHAP teratas, dan riwayat intervensi dalam format yang siap dibagikan.
          </p>
        </div>
        ...
        <aside className="rounded-2xl border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          <h3 className="text-lg font-bold text-dark dark:text-white">Ringkasan Singkat</h3>
          <div className="mt-4 grid gap-3">
            <CardField label="ID Siswa" value={student.id} />
            <CardField label="Prediksi Terbaru" value={formatScore(predictedScore)} />
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <div className="rounded-2xl border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-primary">Faktor prediksi</p>
              <h3 className="mt-1 text-xl font-bold text-dark dark:text-white">Tiga faktor utama pada siswa ini</h3>
            </div>
          ...
          <div className="mt-5 space-y-4">
            {topShapItems.length ? topShapItems.map((item) => (
              <article key={item.feature_name} className="rounded-xl border border-stroke p-4 dark:border-dark-3">
                <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                  <h4 className="text-sm font-semibold text-dark dark:text-white">{item.feature_name}</h4>
              ...
              <p className="text-sm text-dark-4 dark:text-dark-6">Belum ada data SHAP untuk siswa ini.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-stroke bg-white p-6 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-primary">Catatan tindak lanjut</p>
              <h3 className="mt-1 text-xl font-bold text-dark dark:text-white">Riwayat intervensi guru untuk siswa ini</h3>
            </div>
          ...
          <div className="mt-5 space-y-4">
            {interventions.length ? interventions.map((item) => (
              <article key={item.id} className="rounded-xl border border-stroke p-4 dark:border-dark-3">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-semibold text-dark dark:text-white">{item.guru}</p>
                    <p className="text-xs text-dark-4 dark:text-dark-6">{new Date(item.date).toLocaleString("id-ID")}</p>
                  </div>
                </div>
                <p className="mt-3 text-sm leading-6 text-dark-4 dark:text-dark-6">{item.note}</p>
              </article>
            )) : (
              <p className="text-sm text-dark-4 dark:text-dark-6">Belum ada intervensi yang tersimpan untuk siswa ini.</p>
            )}
          </div>
        </div>
      </section>
